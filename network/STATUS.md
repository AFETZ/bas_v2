...
| P4 Interference and medium access | in_progress | A native stock ns-3.40 CSMA baseline is available with bounded native queues/backoff and live Sionna packet errors; the separate centralized priority scheduler with ingress shaping remains available as an explicit custom policy. | Live jammer packet-path integration and dynamic channel-aware admission remain pending. Neither mode is a technology-specific customer modem model. | Add the live jammer path while preserving both separately selectable modes. | `BAS_TOWN01_MEDIUM_ACCESS_MODE=stock_ns3_csma BAS_TOWN01_SKIP_HEATMAPS=1 ./scripts/product/run_town01_full_stack.sh` | `runs/town01-stock-native-final-20260829T193000Z/metrics/medium_access_baseline.json` |
