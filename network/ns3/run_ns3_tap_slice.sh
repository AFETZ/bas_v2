...
STATS_FILE="${STATS_FILE:-$RUN_DIR/metrics/ns3_${PHASE}.json}"
TAP_GCS="${TAP_GCS:-tap-gcs}"
TAP_UAVS="${TAP_UAVS:-tap-uav}"

test -x "$BINARY"
rm -f "$READY_FILE" "$STOP_FILE"
mkdir -p "$RUN_DIR/logs" "$RUN_DIR/metrics" "$RUN_DIR/pcap"
...
exec ip netns exec "$NS3_NS" env \
  ...
  --queueMaxPackets="${QUEUE_MAX_PACKETS:-20}"
